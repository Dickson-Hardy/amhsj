...
async function createMissingTables() {
  try {
    ...
    // Add DOI related columns to articles if missing
    try {
      await sql`ALTER TABLE articles ADD COLUMN IF NOT EXISTS doi_registered BOOLEAN DEFAULT false`
      await sql`ALTER TABLE articles ADD COLUMN IF NOT EXISTS doi_registered_at TIMESTAMP`
      await sql`ALTER TABLE articles ADD COLUMN IF NOT EXISTS crossref_metadata JSONB`
      console.log('✅ Articles table DOI columns updated')
    } catch (error) {
      console.log('⚠️  Some articles columns may already exist')
    }
