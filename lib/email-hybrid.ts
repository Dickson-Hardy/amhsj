...
const VERIFIED_DOMAINS = [
  'yourjournal.com',
  'editorial.yourjournal.com',
  'admin.yourjournal.com'
]

// Email template to category mapping
const TEMPLATE_CATEGORIES: Record<keyof typeof emailTemplates, EmailCategory> = {
  ...
  editorialBoardApplicationReceived: 'editorial',
  editorialBoardApplicationNotification: 'editorial',
  editorialBoardApplicationDecision: 'editorial',
  editorialBoardWelcome: 'editorial'
}
...
async function processEmailQueue() {
  ...
  while (emailQueue.length > 0) {
    ...
    try {
      ...
      console.log(`Email sent successfully to ${job.to} via ${job.provider.toUpperCase()} with ID: ${messageId}`)
    } catch (error: any) {
      console.error(`Failed to send email to ${job.to} via ${job.provider}:`, error)
...
// Remove this function as publicationNotification template doesn't exist
// Use editorialDecision template for publication notifications
export async function sendEditorialDecisionNotification(
  email: string,
  authorName: string,
  articleTitle: string,
  decision: string,
...
export async function checkEmailServiceHealth(): Promise<{
  ...
  try {
    ...
    results.resend = !testResend.error
  } catch (error) {
